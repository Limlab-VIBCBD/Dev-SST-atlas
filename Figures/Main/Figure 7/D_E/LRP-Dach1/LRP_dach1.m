...
datax = x{2, 1}; 
clear x, clear y; 
diff = BG*1.75; %1.75x BG
xline(diff,'-.y', 'LineWidth',2)
xline(maxX,'-.k', 'LineWidth',2)

text(max (datax)*0.6, max (datay)*0.8, sprintf('1.75X BG = %.2f', diff), 'FontSize', 12, 'Color', 'black');
...
Data1(:, 4) = {'YFP-Dach1-low'};%cell type annotation
Data1(idx,4) =  {'YFP-Dach1-high'};%cell type

%%
T = cell2table(Data1);
T.Properties.VariableNames = ["Dach1_int", 'index_col','index_row', 'Celltype', 'YFP_int']; 
T.Celltype = categorical(T.Celltype); 
T.Cellindex = (1:size(T,1)).'; 
T2 = T(T.Celltype == 'YFP-Dach1-high', :); 
T3 = T(T.Celltype == 'YFP-Dach1-low', :); 
...
text(T2.index_col, T2.index_row*1.01, string(T2.Cellindex), 'Color','red','FontSize',10)
plot(T3.index_col, T3.index_row, 'bo', 'MarkerSize',5)
text(T3.index_col, T3.index_row*1.01, string(T3.Cellindex), 'Color','blue','FontSize',10)

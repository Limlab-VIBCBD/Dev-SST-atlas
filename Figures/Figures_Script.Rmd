...
## Packages to install
```{r}
#Chris: had to install rmarkdown, BiocManager, circlize and dittoSeq
...
# Ensure unique genes from majorClassMarkers
selected_genes <- unique(majorClassMarkers$gene)

# Access counts matrix from the layers slot in the RNA assay
counts_matrix <- Atlas_v2@assays$RNA$counts

# Subset counts_matrix using valid genes (genes that exist in the row names of counts_matrix)
valid_genes <- selected_genes[selected_genes %in% rownames(counts_matrix)]

# Create a Seurat object with the subsetted counts matrix
Atlas_Matrix <- CreateSeuratObject(counts = counts_matrix[valid_genes, , drop = FALSE])

# Add metadata from Atlas_v2 based on matching column names
Atlas_Matrix <- AddMetaData(Atlas_Matrix, metadata = Atlas_v2@meta.data[colnames(Atlas_Matrix), , drop = FALSE])
...
install.packages("doMC")  # Install the package
library(doMC)  # Load the package
...
## F ####
#open  Gao et al dataset and check LRP numbers
...
## G ####
Bershteyn <- readRDS('Bershteyn')
DimPlot(Bershteyn, group.by = 'Stage', cols = c('#2AB45D','#0077B6','#28BAA3','#00B9DE','#C574AF'), pt.size = 1)

...
Dataset <- Read10X_h5("Module_1/Data/h5_files/P1_DiBella.h5", use.names = TRUE, unique.features = TRUE)
...
Seurat_object_subset <- subset(seurat_object, subset = nFeature_RNA > 700 & percent.mt < 10 & nCount_RNA > lower.umi)
rm(list = c("logUMI", "seurat_object", "lower.umi"))
gc()
...
s.genes <- readRDS("Module_1/Data/mouse_s.genes.rds")
g2m.genes <- readRDS("Module_1/Data/mouse_g2m.genes.rds")
...
nPCs.Seurat_object_subset <- PCA_estimate_nPC(data.use.Seurat_object_subset, whereto = paste0(path_data,"/optimal_nPCs.RDS"), k = 5, by.nPC = 2, from.nPC = 30, to.nPC = 40) 
rm(path_data)
...
# User define genes to visual which clusters will be selected. We exluded (1) Stress clusters, (2) negative markers - pyramidal, striatal. We selected (3) positve markers - in this case SST, Lhx6 etc
# Specificity plots (UMAP + Violin plot) 
...
VlnPlot(Seurat_object_subset, Contamin, pt.size = 0, group.by = "seurat_clusters")
...
saveRDS(Seurat_object_cleaned, file = "Dataset_P1_Dibella_Sst_filtered.rds") 

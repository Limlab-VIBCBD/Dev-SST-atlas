...
process resolutions_to_test_iteration {

  input:
    tuple val(cl), path(object), path(optimal_nPC)
    val min_resolution
    val max_resolution

  output:
    tuple path(object), path(optimal_nPC), val(cl),path("resolutions_${cl}.csv")
  ...
  write.table(resolution_for_stability, "resolutions_${cl}.csv",quote=FALSE, col.names=FALSE, row.names=FALSE,sep=",")
 """
}
...
process subsampling_and_clustering_iteration {

    input:
      tuple val(resolution), path(object), path(nPCs), val(cl), val(n_subsampling)
      val perc_sub
      val reproducibility

    output:
      tuple val(cl), path("recluster_ident_${resolution}_${n_subsampling}_${cl}.RDS")
    ...
    #!/usr/bin/env Rscript
      ...
      saveRDS(list(recluster_ident,original_ident),"recluster_ident_${resolution}_${n_subsampling}_${cl}.RDS")
    """
  }
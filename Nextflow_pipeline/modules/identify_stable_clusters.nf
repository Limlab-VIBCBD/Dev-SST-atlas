...
process identify_stable_clusters {

    input:
    path recluster_ident_files
    path object
    val jaccard_cutoff
    val percent_cutoff

    output:
    path "Selected_stable_resolution.txt", emit: stable_resolution
    path "subsample_idents.RDS", emit: subsample_ident_file
    path "fullsample_idents.RDS", emit: fullsample_ident_file
    ...
    subsample_idents<-Reduce( "bind_rows", lapply(res, function(x){
        ...
        for(i in 1:n_subsampling){
            out<-readRDS(paste0("recluster_ident_",x,'_',i,".RDS"))
    ...
    if(!(is.null(stable_resolution))){
      ...
      write.table(resolution, "Selected_stable_resolution.txt", row.names = FALSE, col.names = FALSE)
    }else{
      write.table(print("integrated_snn_res.0.0"), "Selected_stable_resolution.txt", row.names = FALSE, col.names = FALSE)
    }
 """
}
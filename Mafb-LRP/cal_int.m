function [Int, BG, Ich1] = cal_int(s, C1, BW)
[M1, num_m] = bwlabel(BW); 
stats = regionprops(M1, 'PixelIdxList', 'Centroid'); 

% read images and background subtract
Ich1 = imread(s(1).inputfile, C1); %channel for TF 
BG = mode (mean(Ich1));%backgound 
%background = imgaussfilt(Ich1, 5); % Adjust the sigma value (20) for scale
%Ich1 = imsubtract(Ich1, background);
